...
export const RepoInputView: React.FC<RepoInputViewProps> = ({ onRepoSubmit, isLoading, error }) => {
  const [repoUrl, setRepoUrl] = useState('');
  const [localError, setLocalError] = useState<string | null>(null);

  // This function is now more robust and correctly handles various GitHub URL formats,
  // trimming trailing slashes and periods.
  const extractRepoName = (url: string): string | null => {
    try {
      ...
      // Trim leading/trailing slashes and get the pathname
      const path = urlObject.pathname.trim().replace(/^\/|\/$/g, '');
      const parts = path.split('/');
      // We need exactly two parts: owner and repo
      if (parts.length >= 2) {
        // Rejoin to handle repo names with dots, then remove potential .git suffix
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!repoName) {
        setLocalError('Please enter a valid GitHub repository URL (e.g., https://github.com/owner/repo).');
        return;
    }
